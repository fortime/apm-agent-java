...
public abstract class AbstractAsyncHttpClientInstrumentation extends TracerAwareInstrumentation {
    ...
    public Collection<String> getInstrumentationGroupNames() {
        return Arrays.asList("http-client", "ninghttpclient");
    }

    @Override
    public ElementMatcher.Junction<ClassLoader> getClassLoaderMatcher() {
        return not(isBootstrapClassLoader())
            .and(classLoaderCanLoadClass("com.ning.http.client.AsyncHandler"));
    }
    ...
    public static class AsyncHttpClientInstrumentation extends AbstractAsyncHttpClientInstrumentation {
        ...
        private static void onBeforeExecute(@Advice.Argument(value = 0) Request request,
            ...
            span = HttpClientHelper.startHttpClientSpan(parent, request.getMethod(), uri.toString(), uri.getScheme(), uri.getHost(), uri.getPort());
...
//            return named("org.asynchttpclient.DefaultAsyncHttpClient");
    ...
    public static class AsyncHandlerOnStatusReceivedInstrumentation extends AbstractAsyncHandlerInstrumentation {
        ...
        public AsyncHandlerOnStatusReceivedInstrumentation(ElasticApmTracer tracer) {
            super(tracer, named("onStatusReceived").and(takesArgument(0, named("com.ning.http.client.HttpResponseStatus"))));
        }

        @Advice.OnMethodEnter(suppress = Throwable.class)
        private static void onMethodEnter(@Advice.This AsyncHandler<?> asyncHandler,
                                          @Advice.Local("span") Span span, @Advice.Argument(0) HttpResponseStatus status) {
        ...
        @Advice.OnMethodExit(suppress = Throwable.class)
        private static void onMethodExit(@Nullable @Advice.Local("span") Span span, @Advice.Argument(0)
